...
class Check:
    ...
    def Run(self):
        ...
        while i<self.count1:
            ...
            if value == -1:
                raise Exception("Error key level_1\nNot found:")
                raise Exception(self.checkdict[i]["name"])
                return -1
            else:
                if type(value) != self.checkdict[i]["type"]:
                    raise Exception("Error value level_1")
                    raise Exception(type(value))
                    raise Exception(value)
                    raise Exception(self.checkdict[i]["type"])
                    return -1
            i=i+1
        ...
        for element in line:
            ...
            while i<self.count2:
                ...
                if value == -1:
                    raise Exception("Error key level_2\nNot found:")
                    raise Exception(self.checkmethods[i]["name"])
                    return -1
                else:
                    if type(value) != self.checkmethods[i]["type"]:
                        raise Exception("Error value level_2")
                        raise Exception(type(value))
                        raise Exception(value)
                        raise Exception(self.checkmethods[i]["type"])
                        return -1
                ...
                for par_el in parameters:
                    ...
                    while j<self.count3:
                        ...
                        if value == -1:
                            raise Exception("Error key level_3\nNot found:")
                            raise Exception(self.checkparameters[j]["name"])
                            return -1
                        else:
                            if type(value) != self.checkparameters[j]["type"]:
                                raise Exception("Error value level_3")
                                raise Exception(type(value))
                                raise Exception(value)
                                raise Exception(self.checkparameters[i]["type"])
                                return -1
                        j=j+1
                        if j==2:
                            j=j+1
